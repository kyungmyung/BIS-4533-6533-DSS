...
stock_price = stock_data[['Close','Volume']]
...
Y_test_re = scaler.inverse_transform(np.column_stack((Y_test, np.zeros(len(Y_test)))))[:, 0]
Y_pred_re= scaler.inverse_transform(np.column_stack((Y_pred, np.zeros(len(Y_pred)))))[:, 0]
