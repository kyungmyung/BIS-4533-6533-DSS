...
stock_price = stock_data[['Open', 'High', 'Low', 'Close', 'Volume']]
...
model = Sequential()
model.add(SimpleRNN(10, activation='tanh', input_shape=(X.shape[1], X.shape[2]), return_sequences=True))
...
Y_test_re = scaler.inverse_transform(np.column_stack((Y_test, np.zeros((len(Y_test), X.shape[2]-1)))))[:, 0]
Y_pred_re= scaler.inverse_transform(np.column_stack((Y_pred, np.zeros((len(Y_pred), X.shape[2]-1)))))[:, 0]
